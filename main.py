...
def FirstLayer(net, l2_reg_val, is_training):
    ...
    tf.losses.add_loss(tf.reduce_sum(net)*l2_reg_val, loss_collection=tf.GraphKeys.REGULARIZATION_LOSSES)
...
def SparseDropout(slice_x, keep_prob=0.5):
    ...
    """
    nz = numpy.nonzero(slice_x)
    nz = nz[0]*slice_x.shape[1]+nz[1]
    nz = numpy.random.permutation(nz)
    r = int(numpy.floor((1-keep_prob)*len(nz)))
    nz = nz[:r]
    x_1d = slice_x.flatten()
    x_1d[nz] = 0
    return x_1d.reshape(slice_x.shape)
...
def ComputeTSNE(embedding_matrix):
    ...
    """
    embeds = TSNE(learning_rate=50, n_iter=500).fit_transform(embedding_matrix)
    return embeds
...
def VisualizeTSNE(sess):
    ...
    class_to_colors = {'positive': 'blue', 'negative': 'orange', 'location': 'green', 'furniture': 'red'}
    for cls in class_to_words:
        for word in class_to_words[cls]:
            i = TERM_INDEX[word]
            plt.scatter(tsne_embeddings[i][0], tsne_embeddings[i][1], c=class_to_colors[cls])
            plt.text(tsne_embeddings[i][0], tsne_embeddings[i][1], s=word)
    plt.show()
...
def BuildInferenceNetwork(x, l2_reg_val, is_training):
    ...
    tfvars = tf.trainable_variables(scope=None)
    EMBEDDING_VAR = tfvars[0]    # ** TASK 4: Move and set appropriately.
