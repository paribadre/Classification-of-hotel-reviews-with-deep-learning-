...
def FirstLayer(net, l2_reg_val, is_training):
    ...
    net = tf.contrib.layers.fully_connected(
            net, 40, activation_fn=None, weights_regularizer=l2_reg, biases_initializer=tf.zeros_initializer())
    net = tf.math.l2_normalize(net, axis=1, epsilon=1e-12, name=None, dim=None)
    net = tf.nn.tanh(net)
    return net
